...
quantitative_features = [q for q in df.columns if q not in categorical_features]
...
functions.plot_dist(
    4, 4, filtered_data[quantitative_features], Transform=["sqrt", "log"]
)
...
# anderson test
alpha_list, stats_list, critical_vals_list = statstical_tests.anderson_test(
    transformed_data
)
...
i = 0
j = 0
for critical_vals, tst_stat, alphas, feature in zip(
    critical_vals_list, stats_list, alpha_list, transformed_data.columns
):
